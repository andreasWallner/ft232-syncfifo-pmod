...
width = base_x + spacing + finger_x
height = 2*spacing + 3*finger_y

footprint_name = f'SolderJumper_2_Tendon_{width}x{height}mm_{spacing}' + ('_bridged' if bridged else '_open')
print(footprint_name)

kicad_mod = Footprint(footprint_name)
kicad_mod.setDescription(f'SMD Solder Jumper, Tendon shape, {width}x{height}mm, {spacing}mm gap, ' + ('bridged' if bridged else 'open'))
kicad_mod.setTags('net tie solder jumper' + (' bridged' if bridged else ' open'))

a = Polygon(nodes=[
        [0, 0],
        [base_x + spacing + finger_x, 0],
        [base_x + spacing + finger_x, -finger_y],
        [base_x                     , -finger_y],
        [base_x                     , -(finger_y*2 + spacing*2)],
        [base_x + spacing + finger_x, -(finger_y*2 + spacing*2)],
        [base_x + spacing + finger_x, -(finger_y*3 + spacing*2)],
        [0                          , -(finger_y*3 + spacing*2)],
    ],
    layer='F.Cu',
    width=0)
...
b = Polygon(nodes=[
        [base_x + spacing           , -(finger_y + spacing)],
        [base_x + spacing + finger_x, -(finger_y + spacing)],
        [base_x + spacing + finger_x, -(finger_y*2 + spacing)],
        [base_x + spacing           , -(finger_y*2 + spacing)]
    ],
    layer='F.Cu',
    width=0)
